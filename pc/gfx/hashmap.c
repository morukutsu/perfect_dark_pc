...
void* nativeMalloc(size_t n) {
    return __builtin_malloc(n);
}

void* nativeRealloc(void* ptr, size_t n) {
    return __builtin_realloc(ptr, n);
}
...
struct hashmap *hashmap_new_with_allocator(
                            ...
                            void *udata)
{
    _malloc = _malloc ? _malloc : nativeMalloc;
    _realloc = _realloc ? _realloc : nativeRealloc;
    _free = _free ? _free : nativeFree;
    int ncap = 16;
    if (cap < ncap) {
        cap = ncap;
    } else {
                            ...
                            void *udata)
{
    return hashmap_new_with_allocator(
        (_malloc?_malloc:nativeMalloc),
        (_realloc?_realloc:nativeRealloc),
        (_free?_free:nativeFree),
