...
u32 setupGetCmdLength(u32 *cmd)
{
	...
	case OBJTYPE_SHIELD:             return sizeof(struct shieldobj) / 4;
	case OBJTYPE_TAG:                return sizeof(struct tag) / 4;
	case OBJTYPE_RENAMEOBJ:          return sizeof(struct textoverride) / 4;
	...
	case OBJTYPE_BRIEFING:           return sizeof(struct briefingobj) / 4;
	case OBJTYPE_PADLOCKEDDOOR:      return 4;
	case OBJTYPE_TRUCK:              return 34;
	case OBJTYPE_HELI:               return 35;
	case OBJTYPE_TANK:               return 32;
	case OBJTYPE_CAMERAPOS:          return sizeof(struct cameraposobj) / 4;
	case OBJTYPE_LIFT:               return sizeof(struct liftobj) / 4;
	case OBJTYPE_CONDITIONALSCENERY: return sizeof(struct linksceneryobj) / 4;
	...
	case OBJTYPE_FAN:                return sizeof(struct fanobj) / 4;
	case OBJTYPE_HOVERCAR:           return sizeof(struct hovercarobj) / 4;
